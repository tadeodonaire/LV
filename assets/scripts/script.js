...
document.querySelectorAll('.calculate-btn').forEach(button => {
  button.addEventListener('click', function() {
      ...
      const articleInput = card.querySelector('.article-input');
      const calculatedPriceElement = card.querySelector('.calculated-price');
      
      let price = 0;

      // Calculate price for weight input
      if (weightInput && weightInput.value) {
          const weight = parseFloat(weightInput.value);
          price = (28.32 / 6) * weight; // Price calculation for weight service
      } 
      // Calculate price for article input
      else if (articleInput && articleInput.value) {
          const quantity = parseInt(articleInput.value);
          price = 3 * quantity; // Price calculation for article service
      }

      // Update calculated price in the card
      calculatedPriceElement.textContent = `S/ ${price.toFixed(2)}`;
  });
});
